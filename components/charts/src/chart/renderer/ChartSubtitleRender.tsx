...
/**
 * `ChartSubTitleRenderer` is a functional component used to render the subtitle of the chart.
 * It participates in the layout system and reports its measurements during the measuring phase.
 *
 * @param {ChartTitleProps} props - Contains properties required for rendering the chart subtitle,
 ...
 */
export const ChartSubTitleRenderer: React.ForwardRefExoticComponent<ChartTitleProps & React.RefAttributes<SVGTextElement>> =
    forwardRef<SVGTextElement, ChartTitleProps>((props: ChartTitleProps, ref: React.ForwardedRef<SVGTextElement>) => {
            ...
            if (phase === 'measuring') {
                if (props.text) {
                    ...
                    const titleCollection: string[] = getTitle(
                        props.text, titleStyle, ((props.position === 'Top' || props.position === 'Bottom') ? rect.width : rect.height),
                        chart.enableRtl, chart.themeStyle.chartSubTitleFont, (props.textOverflow as TextOverflow));
                    const titleOptions: TitleOptions = computeSubtitlePosition(chart, props, titleStyle, titleCollection);
                    chart.subTitleSettings = titleOptions;
                }
                reportMeasured('ChartSubsTitle');
            }
        }, [phase, layoutRef]);
        useEffect(() => {
            if (phase !== 'measuring') {
        ...
        return (phase === 'rendering' && props.text) && (
            <>
                {subTitleBorder((layoutRef.current.chart as Chart), props)}
                {renderSubTitleElement((layoutRef.current.chart as Chart), props, ref)}
            </>
        );
    });
 ...
 * @param {ChartTitleProps} subtitleProps - The style settings for the subtitle text.
 * @param {TextStyleModel} textStyle - The text style model for the subtitle.
 ...
 * @private
 */
function computeSubtitlePosition(chart: Chart, subtitleProps: ChartTitleProps, textStyle: TextStyleModel,
                                 subTitle: string[]): TitleOptions {
    const margin: MarginModel = chart.margin;
    let subTitleSize: ChartSizeProps;
    ...
    case 'Left':
        ...
        positionY = subtitleProps.align === 'Left' ? margin.bottom + (subtitleBorderWidth * 0.5) + chart.border.width :
            subtitleProps.align === 'Right' ? chart.availableSize.height - margin.bottom - (subtitleBorderWidth * 0.5) - chart.border.width : chart.availableSize.height / 2;
 ...
 * @param {ChartTitleProps} titleProps - Properties that provide details about the chart subtitle.
 * @returns {JSX.Element} A rectangle SVG element representing the border around the subtitle.
 * @private
 */
function subTitleBorder(chart: Chart, titleProps: ChartTitleProps): JSX.Element {
 ...
 *
 * @param {Chart} chart - The chart instance to which the subtitle belongs.
 * @param {ChartTitleProps} titleProps - Properties that define the subtitle characteristics in the chart.
 ...
 * @private
 */
function renderSubTitleElement(chart: Chart, titleProps: ChartTitleProps, ref: ForwardedRef<SVGTextElement>): JSX.Element {
    ...
    return (subTitleOptions) && (
        <text
            ...
            fill={titleProps.color || chart.themeStyle.chartSubTitleFont.color}
            fontFamily={titleProps.fontFamily || chart.themeStyle.chartSubTitleFont.fontFamily}
            fontSize={titleProps.fontSize || chart.themeStyle.chartSubTitleFont.fontSize}
            fontWeight={titleProps.fontWeight || chart.themeStyle.chartSubTitleFont.fontWeight}
