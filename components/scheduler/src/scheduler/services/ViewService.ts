...
export class ViewService {
    ...
    /**
     * Check if a specific view type is available using a getter
     *
     * @param {ViewsInfo[]} getAvailableViews - Function returning available views
     * @param {View} viewType - View type to check (e.g., 'Day')
     * @returns {boolean} True if the view type exists in available views
     */
    static isViewAvailable(
        getAvailableViews: () => ViewsInfo[] | undefined,
        viewType: View
    ): boolean {
        const views: ViewsInfo[] | undefined = getAvailableViews?.();
        return Array.isArray(views) && views.some((v: ViewsInfo) => v.viewType === viewType);
    }

    /**
     * Convenience helper to check if Day view is available
     *
     * @param {ViewsInfo[]} getAvailableViews - Function returning available views
     * @returns {boolean} True if Day view exists in available views
     */
