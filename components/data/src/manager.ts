...
import { IFetch, Fetch } from '@syncfusion/react-base';
...
export class DataManager {
    ...
    private requests: IFetch[] = [];
    private fetchDeffered: Deferred;
    private fetchReqOption: IFetch;
    ...
    private makeRequest(url: ReturnOption, deffered: Deferred, args?: RequestOptions, query?: Query): Object {
        ...
        const fnSuccess: Function = (data: string | Object, request: IFetch) => {
            ...
            if (this.isCustomDataAdaptor(this.adaptor)) {
                request = extend({}, this.fetchReqOption, request) as IFetch;
            }
        ...
        if (!this.isCustomDataAdaptor(this.adaptor)) {
            ...
            this.fetchReqOption = req as IFetch;
    ...
    private beforeSend(request: Request, settings?: IFetch, response?: Object): void {
    ...
    public saveChanges(
        ...
        if (!isNullOrUndefined(this.adaptor[dofetchRequest])) {
            ...
            const fetch: IFetch = Fetch(req);
            fetch.beforeSend = () => {
                this.beforeSend(fetch.fetchRequest, fetch);
            };
            fetch.onSuccess = (data: string | Object, request: IFetch) => {
                if (this.isGraphQLAdaptor(this.adaptor)) {
                    // tslint:disable-next-line:no-string-literal
                    if (!isNullOrUndefined(data['errors'])) {
                        // tslint:disable-next-line:no-string-literal
                        fetch.onFailure(new Error(JSON.stringify(data['errors'])));
                    }
                }
                deff.resolve(this.adaptor.processResponse(
                    data, this, null, request.fetchRequest, request, changes, args));
            };
            fetch.onFailure = (e: Error) => {
                deff.reject([{ error: e.message }]); 
            };;
    ...
    private successFunc(record: string | Object, request: IFetch, changes?: Object): void {
        if (this.isGraphQLAdaptor(this.adaptor)) {
    ...
    private dofetchRequest(res: Object, fetchFunc?: Function, changes?: Object): Promise<Response> {
        ...
        if (!this.isCustomDataAdaptor(this.adaptor)) {
            ...
            this.fetchReqOption = res as IFetch;
            fetchFunc.call(this, {
